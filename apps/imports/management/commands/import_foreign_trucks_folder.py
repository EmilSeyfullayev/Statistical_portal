...
class Command(BaseCommand):
    help = "Register and import every Foreign Trucks Excel workbook into foreign_trucks tables."
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "--path",
            default="",
            help="Override the Foreign Trucks upload folder. Defaults to the seeded data source destination.",
        )
        parser.add_argument(
            "--skip-merge",
            action="store_true",
            help="Do not rebuild foreign_trucks.foreign_trucks_merged after importing files.",
        )

    def handle(self, *args, **options):
        data_source = self.get_foreign_trucks_data_source()
        source_dir = self.source_directory(data_source, options["path"])
        if not source_dir.exists():
            raise CommandError(f"Foreign Trucks folder does not exist: {source_dir}")

        importer_class = get_importer(data_source.parser_key)
        files = self.foreign_trucks_files(source_dir)
        if not files:
            self.stdout.write(self.style.WARNING(f"No Foreign Trucks Excel files found in {source_dir}"))
            return
        ...
        for path in files:
            ...
            if job.status == ImportJob.STATUS_SUCCESS:
                ...
                self.stdout.write(
                    f"Imported {path.relative_to(source_dir)} -> foreign_trucks.{table_name} "
        ...
        summary = (
            f"Foreign Trucks import complete: {totals['imported']} imported, "
    ...
    def get_foreign_trucks_data_source(self):
        SeedCommand().handle()
        data_source = DataSource.objects.filter(
            parser_key="foreign_trucks_excel_v1",
            submodule__name="Xarici TIR-lar",
        ).select_related("submodule__module").first()
        if not data_source:
            raise CommandError("Foreign Trucks data source was not found after seeding.")
        return data_source
    ...
    def foreign_trucks_files(self, source_dir):
