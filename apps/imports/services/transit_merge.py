from dataclasses import dataclass

from django.db import connection

TRANSIT_SCHEMA = "transit"
MERGED_TABLE = "transit_merged"
PORTAL_TABLE = "transit_data_for_portal"
SORT_COLUMN = "merged_cixis_tarixi"
EXCLUDED_SOURCE_TABLES = {MERGED_TABLE, PORTAL_TABLE}

# Per-file tables skipped by UNION ALL when building transit_merged.
# transit_2026_2026_01 is omitted because the next month's file (2026_02) is
# cumulative and already includes January 2026 data; merging both would double-count.
OMITTED_SOURCE_TABLES: dict[str, str] = {
    "transit_2026_2026_01": (
        "Omitted because the next month's file (2026_02) is cumulative and "
        "already includes January 2026 data."
    ),
}

NUMERIC_COLUMNS = {"source_row_number", "import_job_id"}
DATE_COLUMNS = {"giris_tarixi", "cixis_tarixi"}
TIMESTAMP_COLUMNS = {"imported_at", *DATE_COLUMNS}
US_DATE_TEXT_PATTERN = r"^\d{1,2}/\d{1,2}/\d{4}$"


@dataclass(frozen=True)
class MergeResult:
    source_table_count: int
    merged_row_count: int
    merged_table: str


def quote_name(name):
    return connection.ops.quote_name(name)


def qualified_name(schema, table):
    return f"{quote_name(schema)}.{quote_name(table)}"


def source_tables(cursor):
    cursor.execute(
        """
        SELECT table_name
        FROM information_schema.tables
        WHERE table_schema = %s
          AND table_type = 'BASE TABLE'
          AND table_name LIKE 'transit_%%'
          AND table_name <> ALL(%s)
        ORDER BY table_name
        """,
        [TRANSIT_SCHEMA, list(EXCLUDED_SOURCE_TABLES)],
    )
    return [
        table_name
        for table_name in (row[0] for row in cursor.fetchall())
        if table_name not in OMITTED_SOURCE_TABLES
    ]


def table_columns(cursor, tables):
    if not tables:
        return {}
    cursor.execute(
        """
        SELECT table_name, column_name, data_type, ordinal_position
        FROM information_schema.columns
        WHERE table_schema = %s
          AND table_name = ANY(%s)
        ORDER BY table_name, ordinal_position
        """,
        [TRANSIT_SCHEMA, tables],
    )
    columns_by_table = {table: [] for table in tables}
    for table_name, column_name, data_type, _ordinal_position in cursor.fetchall():
        columns_by_table[table_name].append((column_name, data_type))
    return columns_by_table


def merged_columns(columns_by_table):
    seen = set()
    columns = []
    for table_columns_list in columns_by_table.values():
        for column_name, _data_type in table_columns_list:
            if column_name not in seen:
                seen.add(column_name)
                columns.append(column_name)
    return columns


def merged_type_for(column_name):
    if column_name in NUMERIC_COLUMNS:
        return "bigint"
    if column_name in TIMESTAMP_COLUMNS:
        return "timestamp with time zone"
    return "text"


def date_cast_expression(column_name, data_type):
    quoted_column = quote_name(column_name)
    if data_type == "timestamp with time zone":
        return f"{quoted_column}::timestamp with time zone"

    trimmed = f"NULLIF(btrim({quoted_column}::text), '')"
    return (
        "CASE "
        f"WHEN {trimmed} IS NULL THEN NULL "
        f"WHEN {trimmed} ~ "
        f"'{US_DATE_TEXT_PATTERN}' "
        f"THEN to_timestamp({trimmed}, 'MM/DD/YYYY') "
        "ELSE NULL "
        "END"
    )


def cast_expression(column_name, data_type):
    quoted_column = quote_name(column_name)
    if column_name in NUMERIC_COLUMNS:
        return f"{quoted_column}::bigint"
    if column_name in DATE_COLUMNS:
        return date_cast_expression(column_name, data_type)
    if column_name in TIMESTAMP_COLUMNS:
        return f"{quoted_column}::timestamp with time zone"
    return f"{quoted_column}::text"


def sort_expression(source_columns):
    if "cixis_tarixi" not in source_columns:
        return "NULL::timestamp with time zone"
    return date_cast_expression("cixis_tarixi", source_columns["cixis_tarixi"])


def select_sql_for_table(table, columns, table_column_map):
    source_columns = {column_name: data_type for column_name, data_type in table_column_map}
    select_parts = [f"{sort_expression(source_columns)} AS {quote_name(SORT_COLUMN)}"]
    for column in columns:
        if column in source_columns:
            expression = cast_expression(column, source_columns[column])
        else:
            expression = f"NULL::{merged_type_for(column)}"
        select_parts.append(f"{expression} AS {quote_name(column)}")
    return f"SELECT {', '.join(select_parts)} FROM {qualified_name(TRANSIT_SCHEMA, table)}"


def create_merged_table(cursor, tables, columns_by_table):
    columns = merged_columns(columns_by_table)
    merged_columns_sql = ", ".join(
        f"{quote_name(column)} {merged_type_for(column)}" for column in columns
    )
    cursor.execute(f"DROP TABLE IF EXISTS {qualified_name(TRANSIT_SCHEMA, MERGED_TABLE)}")
    cursor.execute(
        f"CREATE TABLE {qualified_name(TRANSIT_SCHEMA, MERGED_TABLE)} "
        f"({quote_name(SORT_COLUMN)} timestamp with time zone, {merged_columns_sql})"
    )
    union_sql = "\nUNION ALL\n".join(
        select_sql_for_table(table, columns, columns_by_table[table]) for table in tables
    )
    cursor.execute(
        f"INSERT INTO {qualified_name(TRANSIT_SCHEMA, MERGED_TABLE)} "
        f"SELECT * FROM ({union_sql}) merged_source "
        f"ORDER BY {quote_name(SORT_COLUMN)} DESC NULLS LAST"
    )
    cursor.execute(
        f"CREATE INDEX IF NOT EXISTS {quote_name(f'{MERGED_TABLE}_{SORT_COLUMN}_idx')} "
        f"ON {qualified_name(TRANSIT_SCHEMA, MERGED_TABLE)} ({quote_name(SORT_COLUMN)} DESC NULLS LAST)"
    )
    cursor.execute(
        f"CREATE INDEX IF NOT EXISTS {quote_name(f'{MERGED_TABLE}_source_file_path_idx')} "
        f"ON {qualified_name(TRANSIT_SCHEMA, MERGED_TABLE)} ({quote_name('source_file_path')})"
    )


def rebuild_transit_merged():
    with connection.cursor() as cursor:
        cursor.execute(f"CREATE SCHEMA IF NOT EXISTS {quote_name(TRANSIT_SCHEMA)}")
        tables = source_tables(cursor)
        if not tables:
            cursor.execute(f"DROP TABLE IF EXISTS {qualified_name(TRANSIT_SCHEMA, MERGED_TABLE)}")
            return MergeResult(source_table_count=0, merged_row_count=0, merged_table=f"{TRANSIT_SCHEMA}.{MERGED_TABLE}")

        columns_by_table = table_columns(cursor, tables)
        create_merged_table(cursor, tables, columns_by_table)
        cursor.execute(f"SELECT count(*) FROM {qualified_name(TRANSIT_SCHEMA, MERGED_TABLE)}")
        row_count = cursor.fetchone()[0]
    return MergeResult(
        source_table_count=len(tables),
        merged_row_count=row_count,
        merged_table=f"{TRANSIT_SCHEMA}.{MERGED_TABLE}",
    )
